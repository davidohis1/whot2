...
class WhotLobbyScreen extends ConsumerStatefulWidget {
  final int playerCount; // 2 or 4, passed via GoRouter extra
  const WhotLobbyScreen({super.key, this.playerCount = 4});
...
class _WhotLobbyScreenState extends ConsumerState<WhotLobbyScreen> {
  ...
  int get _pc => widget.playerCount;
  ...
  Future<void> _play() async {
    ...
    try {
      final result = await ref.read(whotServiceProvider)
          .joinOrCreate(user.username, playerCount: _pc);
      if (!mounted) return;
  ...
  Widget _header(user) => Padding(
    ...
    child: Row(children: [
      ...
      Text('🃏  Whot · ${_pc}P', style: const TextStyle(
  ...
  Widget _gameCard() {
    final prizeText = _pc == 2
        ? 'Winner takes all'
        : '1st: 1000 🪙  ·  2nd: 300 🪙';
    final winChip   = _pc == 2 ? '700 🪙' : '1000 🪙';
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Text('$_pc Players · 7 Minute Timer · Landscape',
  ...
  Widget _warningBanner() {
    final msg = _pc == 2
        ? '400 coins will be deducted from both players once the game starts. '
          'The game begins automatically when 2 players join.'
  ...
  Widget _waitingView() {
    final lobbies = ref.watch(whotLobbyProvider(_pc)).valueOrNull ?? [];
    ...
    return Center(
      child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        Row(mainAxisAlignment: MainAxisAlignment.center,
          children: List.generate(_pc, (i) => Container(
            margin: const EdgeInsets.all(6),
            width: 56, height: 56,
            decoration: BoxDecoration(
              color: i < count
                  ? AppColors.teal.withOpacity(0.2) : AppColors.bg2,
              borderRadius: BorderRadius.circular(12),
              border: Border.all(
        ...
        Text('$count / $_pc players joined', style: const TextStyle(
  ...
  Widget _openLobbies() {
    ...
    return Column(
      ...
      children: [
        ...
        ...lobbies.map((l) =>
            _LobbyTile(l).animate().fadeIn().slideX(begin: 0.1)),
      ],
    );
  }
}
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Row(children: [
      ...
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Text('${lobby.players.length}/${lobby.playerCount} players',
